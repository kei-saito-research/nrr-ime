...
% TITLE AND AUTHOR
%==============================================================================
\title{\textbf{NRR-IME: Structure-Aware Optimization for\\Stateful Reasoning on Stateless LLM APIs}}
...
across turns. We present IME (Interpretation Management Engine), a concrete NRR
implementation for stateless LLM APIs, and compare six implementation strategies.
In the main benchmark (135 runs = 45 configurations $\times$ 3 trials; Claude
Sonnet 4, GPT-4o-mini, Gemini 2.0 Flash), Phase~1.5 (operator abstraction) is
best-performing under this protocol: 100\% operator extraction with near-zero
token variance (std $\leq$ 0.5 at temperature 0.3). The other strategies expose
four failure modes: unconstrained output variance/extraction failure (Phases~1.0
...
regions. NRR does not replace standard LLM use; it optimizes when to commit and
when to defer under explicit conditions. IME integrates into existing pipelines
without model-side architectural changes.
...
computational efficiency, LLM optimization, state management, operator abstraction
...
This paper introduces \textbf{IME (Interpretation Management Engine)}, the first concrete
implementation of NRR principles, which maintains weighted distributions over semantic
interpretations in conversational contexts.

When implementing IME on stateless LLM APIs, a fundamental design question arises:
\textbf{what should the LLM be asked to return?} The answer defines a design space
spanning six distinct strategies, from full JSON state reconstruction to binary operator
selection to hybrid keyword-LLM routing. Each strategy embodies different assumptions
about what LLMs can reliably do.

This paper systematically explores this design space through 135 experimental runs
across three commercial LLM APIs, revealing that each strategy fails in a characteristic
way---except one.

\subsection{Contributions}

\begin{enumerate}
    \item \textbf{Design space mapping}: Six implementation strategies for stateful
    reasoning on stateless APIs, spanning representative points in the ``what to send'' $\times$
    ``what to return'' design space.
    \item \textbf{Cross-model validation}: All strategies tested on Claude Sonnet~4,
    GPT-4o-mini, and Gemini~2.0~Flash with 3 trials each (temperature~0.3), providing
    mean $\pm$ standard deviation for all metrics.
    \item \textbf{Failure taxonomy}: Four distinct failure modes identified---output
    variance, weight oscillation, semantic collapse, and input dependency---each
    demonstrating a different limitation of LLM-based state management.
    \item \textbf{Structural stability discovery}: Phase~1.5 achieves near-zero variance
    across trials despite non-zero temperature, demonstrating that prompt design can
    create structurally stable outputs from stochastic systems.
\end{enumerate}
...
Among the tested strategies, Phase~1.5 (operator abstraction) is the only one that simultaneously achieves:
(a)~100\% extraction reliability across all models and scenarios,
(b)~near-zero output variance across repeated trials (max std $\leq$ 0.5 tokens),
(c)~controlled state evolution through fixed-step updates, and
(d)~preservation of multiple interpretations in superposition.
No other tested strategy satisfies all four requirements under this protocol.
...
IME is the first concrete implementation of NRR principles for tracking semantic
interpretation evolution in conversational contexts. For example, the word ``bank''
...
this work by identifying a specific mechanism---output space compression---through
which prompt design achieves reproducibility.
...
\caption{Design space of implementation strategies. Green = optimal (no failure),
orange = partial failure (severity varies: Phase~1.3 produces valid output format but unstable weights; Phase~2.0 destroys multi-interpretation state entirely), red = complete failure.}
...
1.5 & Labels only & Binary operator & \textbf{Optimal} \\
...
\begin{principle}[Format as Stability Mechanism]
Output format constraints serve not just as parsing aids but as stability
mechanisms. Phase~1.5's near-zero variance is a direct consequence of its format
constraint, not of temperature settings.
\end{principle}
...
discussion. This is not a trivial result---it means that \textbf{prompt design
alone can create near-deterministic behavior from a stochastic system.}
...
stochastic APIs, \textbf{shrink the output space rather than lowering the
temperature.} Lowering temperature alone does not reduce output space size;
Phase~1.5 achieves stability through structural constraint on what can be
generated, not through sampling suppression.
...
work should test targeted prompt/format controls and session-aware calibration
...
This paper presents a systematic exploration of six implementation strategies
for NRR-IME on stateless LLM APIs. In the main 135-run comparison across three
models, Phase~1.5 (operator abstraction) is best-performing under this protocol:
it is the only tested strategy that simultaneously achieves 100\% extraction
reliability, near-zero output variance, controlled state evolution, and
preservation of semantic ambiguity.
...
output variance from unconstrained responses---1.0 through verbose JSON, 1.6
through insufficient instruction. Phase~1.3 shows unstable incremental updates
(bang-bang oscillation) in the tested prompt format. Phase~2.0 tends to collapse
...
The key insight is architectural: \textbf{constrain the LLM to the minimum
decision space and delegate all computation to the client.} In our setup, this principle
supports reliable, reproducible stateful behavior on stochastic, stateless APIs.
In this setup, the gains come from reducing unnecessary early commitment and preserving reversible update paths, rather than adding architectural complexity.
...
models---including Claude (Anthropic), ChatGPT and Codex (OpenAI), and 
Gemini (Google)---for their assistance in linguistic refinement, 
