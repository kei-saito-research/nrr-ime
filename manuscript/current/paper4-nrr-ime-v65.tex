...
stateless APIs: stable state transitions emerge when semantic decision is kept
model-side and deterministic execution is externalized to the client. NRR does
not replace standard LLM use; it optimizes when to commit and when to defer
...
this work by identifying a specific mechanism---interface compression---through
which return-structure constraint improves reproducibility.
